...
module.exports = {
    getPosts: async (req, res) => {
        try {
            const result = await db.query('SELECT p.id, title, content, image_url, user_id, post_type_id, created_at, name FROM posts p JOIN post_types pt ON pt.id = p.post_type_id ORDER BY created_at DESC');
