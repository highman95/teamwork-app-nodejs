module.exports = {
  ...
  async create(postTypeId, title, content, imageUrl, userId) {
    ...
    try {
      ...
      const result = await db.query(
        `INSERT INTO posts (title, content, image_url, post_type_id, user_id) VALUES ($1, $2, $3, $4, $5) RETURNING ${returnColumns}`,
        inputs
      );
  ...
  fetchAll: async (postTypeId, page = 1, chunk = 10) => {
    ...
    try {
      // eslint-disable-next-line no-undef
      const result = await db.query(
        `SELECT id, title, content, image_url, user_id, post_type_id, created_at FROM posts WHERE 1 = 1 ${where} ORDER BY created_at DESC LIMIT ${limit} OFFSET ${offset}`,
        filter
      );
      return result.rows;
    } catch (e) {
